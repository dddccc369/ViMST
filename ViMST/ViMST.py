...
class ViMST:
    def __init__(self,
        ...
        self.__img_mask_rate = img_mask_rate
    ...
    def train(self):
        ...
        self.model = ViMST_model(features_dims, bn=self.__bn,
                                  ...
                                  edge_drop_rate=self.__edge_drop_rate, feat_mask_rate=self.__feat_mask_rate,img_mask_rate=self.__img_mask_rate,
        ...
        for epoch in epoch_iter:
            ...
            feat_loss, img_loss, topo_loss = self.model(data)
            #对loss进行重新的计算，加上img_loss
            loss = feat_loss * 0.7 + img_loss * 0.3  + topo_loss * self.__lam 
            loss.backward()
    ...
    def process(self, method="kmeans"):
        ...
        with torch.no_grad():
            self.model.eval()
            h, h1, z, z1 = self.model.recon(data=data)
            #潜在基因
            rep = h.to('cpu').detach().numpy()
            #潜在图像
            rep1 = h1.to('cpu').detach().numpy()
            #重建基因
            rec = z.to('cpu').detach().numpy()
            #重建图像
            rec1 = z1.to('cpu').detach().numpy() 
            if rep.shape[-1] > 64:
            ...
            #将潜在基因和潜在图像进行一个合并           
            self.__adata.obsm["eval_pred"] = np.concatenate((rep * self.__n1 , rep1 * self.__n2), axis=1)
            
            ret = rec * 0.7 + rec1 * 0.3
            from sklearn.preprocessing import StandardScaler
            ret = StandardScaler().fit_transform(ret)
            self.__adata.obsm["eval_recon"] = ret
            if method == "mclust":
